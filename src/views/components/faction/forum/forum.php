    ...
    echo '<div class="fix-body">';
        echo '<div class="body">';
            ...
            for ($i = 1; $i <= ForumResources::size(); ++$i) {
                if ((ForumResources::getInfo($i, 'id') < 10) || (ForumResources::getInfo($i, 'id') >= 10 && ForumResources::getInfo($i, 'id') < 20 && $session->get('playerInfo')->get('status') > 2) || (ForumResources::getInfo($i, 'id') >= 20 && ForumResources::getInfo($i, 'id') < 30 && Player::CHIEF == $session->get('playerInfo')->get('status'))) {
                    $active = ((!$request->query->has('forum') and 1 == $i) or $request->query->get('forum') == ForumResources::getInfo($i, 'id')) ? 'active' : '';
