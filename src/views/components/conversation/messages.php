...
for ($i = 0; $i < $conversationMessageManager->size(); ++$i) {
    ...
    if (0 != $i and $conversationMessageManager->get($i - 1)->dCreation > $dPlayerLastMessage and $m->dCreation <= $dPlayerLastMessage) {
        echo '<div class="system-message">';
        echo 'Dernier message lu';
        echo '</div>';
    }

    if (ConversationMessage::TY_STD == $m->type) {
...
if (ConversationMessage::MESSAGE_BY_PAGE == $conversationMessageManager->size()) {
