...
if (false !== $discordId and '' !== $discordId) {
    $chickenBot = 'http://chickenbot.cloudapp.net:8080/register';

    $data = ['discordId' => $discordId, 'server' => 's'.$this->getContainer()->getParameter('server_id'), 'username' => $session->get('playerInfo')->get('name'), 'factionColor' => $session->get('playerInfo')->get('color')];
