...
class CommanderRepository extends AbstractRepository
{
    ...
    public function getIncomingAttacks($playerId)
    {
        $statement = $this->select(
            ...
            'AND c.travelType IN ('.Commander::COLO.', '.Commander::LOOT.')', ['player_id' => $playerId]);
        $commanders = [];
        $currentId = 0;
        $persisted = [];
        $unPersisted = [];
    ...
    public function getOutcomingAttacks($playerId)
    {
        $statement = $this->select(
            ...
            'AND c.statement = '.Commander::MOVING, ['player_id' => $playerId]);
        $commanders = [];
        $currentId = 0;
        $persisted = [];
        $unPersisted = [];
    ...
    {
        $statement = $this->select(
            'WHERE c.rDestinationPlace = :place_id AND c.statement = '.Commander::MOVING.' ORDER BY c.dArrival ASC', ['place_id' => $placeId]);
        $commanders = [];
        $currentId = 0;
        $persisted = [];
        $unPersisted = [];
    ...
    public function getFactionCommanderStats(int $factionId): array
    {
		...
		');
        $qr->execute([$factionId, Commander::AFFECTED, Commander::MOVING]);
        $aw1 = $qr->fetch();
        $qr->closeCursor();
    ...
    {
        if (null === $currentId || (int) $data['id'] !== $currentId) {
            $currentId = $data['id'];
