...
class LiveReportRepository extends AbstractRepository
{
    ...
    {
        $statement = $this->select(
            'r.rPlayerAttacker = ? AND r.rPlace IN ('.implode(',', $places).') ORDER BY r.dFight DESC LIMIT 30', [$playerId]);
        $data = [];
        while ($row = $statement->fetch()) {
            $data[] = $this->format($row);
        }
    ...
    {
        $statement = $this->select(
            'rPlayerAttacker = ? AND statementAttacker = ? '.(($hasRebels) ? '' : 'AND p2.rColor != 0').' ORDER BY dFight DESC LIMIT 0, 50', [$playerId, $isArchived]);
        $data = [];
        while ($row = $statement->fetch()) {
            $data[] = $this->format($row);
        }
    ...
    {
        $statement = $this->select(
            'rPlayerDefender = ? AND statementDefender = ? '.(($hasRebels) ? '' : 'AND p2.rColor != 0').' ORDER BY dFight DESC LIMIT 0, 50', [$playerId, $isArchived]);
        $data = [];
        while ($row = $statement->fetch()) {
            $data[] = $this->format($row);
        }
    ...
    {
        $statement = $this->select(
            'p1.rColor = ? AND p2.rColor != 0 ORDER BY dFight DESC LIMIT 0, 30', [$factionId]);
        $data = [];
        while ($row = $statement->fetch()) {
            $data[] = $this->format($row);
        }
    ...
    {
        $statement = $this->select(
            'p2.rColor = ? AND p1.rColor != 0 ORDER BY dFight DESC LIMIT 0, 30', [$factionId]);
        $data = [];
        while ($row = $statement->fetch()) {
            $data[] = $this->format($row);
        }
